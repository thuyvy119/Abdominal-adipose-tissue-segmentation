...
from utils import *
import torch
from dataset_m2 import CTFatDataset
...
plots_path = '/Users/vynguyen/Documents/VSCode/Abdominal_segmentation/model 2/plot_pred1'

def plot_results(img_np, mask_np, y_pred_np, loss, epoch, name_loss):
    fig, axs = plt.subplots(1, 3, figsize=(15, 5))
    axs[0].imshow(img_np[0].squeeze(), cmap='gray')
    axs[0].set_title('Image')
    axs[0].axis('off')
    
    axs[1].imshow(mask_np[0].squeeze(), cmap='jet')
    axs[1].set_title('Mask')
    axs[1].axis('off')
    
    axs[2].imshow(y_pred_np[0].squeeze(), cmap='jet')
    axs[2].set_title(f'Predicted Result\nDice Loss: {loss:.4f}')
    axs[2].axis('off')
    
    plt.tight_layout()
    
    plot_filename = os.path.join(plots_path, f'{name_loss}_epoch_{epoch}.png')
...
def main():
    ...
    for epoch in range(n_epochs):
        ...
        if phase == "valid":
            ...
            # Plot results every 40 epochs
            if (epoch + 1) % 40 == 0:
                for idx in range(len(val_pred)):
                    plot_results(img_np, mask_np, y_pred_np, val_loss[idx], epoch, name_loss)
